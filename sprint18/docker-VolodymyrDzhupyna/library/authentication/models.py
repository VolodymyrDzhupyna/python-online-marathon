...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    def get_absolute_url(self):
        return reverse("users:single", kwargs={"pk": self.pk})
    ...
    def validate(self):
        """
        Make validation for user instance data.
        :return: None if all data are valid otherwise dict of all fields errors
        """
        dict_of_errors = dict()
        try:
            self.full_clean()
            ...
            for item in e:
                dict_of_errors[item[0]] = '<br/>'.join(item[1])

        try:
            validate_password(self.password)
        except ValidationError as e:
            if dict_of_errors.get('password'):
                dict_of_errors['password'] += '<br/>'+'<br/>'.join(e)
            else:
                dict_of_errors['password'] = ' <br/>'.join(e)
        if dict_of_errors:
