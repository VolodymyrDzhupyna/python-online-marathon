...
class Author(models.Model):
    ...
    @staticmethod
    def get_by_fullname(author_fullname):
        name_parts = author_fullname.split(' ')
        if len(name_parts) == 3:
            name, patronymic, surname = author_fullname.split(' ')
        elif len(name_parts) == 2:
            name, surname = author_fullname.split(' ')
            patronymic = ''
        try:
            author = Author.objects.get(name=name, surname=surname, patronymic=patronymic)
            return author
        except:
